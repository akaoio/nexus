...
│                     (sqlite-wasm │ better-sqlite3 │ libSQL  │
...
- Engine hỗ trợ từ v1, ngang hàng: **SQLite** (better-sqlite3 server / `@sqlite.org/sqlite-wasm` browser), **libSQL** (dialect chính thức, mở đường embedded replicas + edge sync), **PostgreSQL**, **MySQL/MariaDB**.
- **Ghi chú libSQL/WASM** (đã kiểm chứng, tránh nhầm): `@libsql/client-wasm` chủ yếu là client HTTP tới server libSQL từ môi trường WASM — **không** phải SQLite cục bộ trong browser. Cái chạy local-WASM thật là **Turso Database** (tên cũ "Limbo") — rewrite Rust **đang beta**, dự kiến tương lai sẽ *trở thành* libSQL. Quyết định: local mode dùng `@sqlite.org/sqlite-wasm` (đã chạy production trong akao); libSQL là engine server; theo dõi Turso DB như ứng viên thay thế local engine khi GA — nhờ ranh giới dialect, việc đổi sau này không chạm app nào.
...
| libSQL | **Native trong core** — vector type + DiskANN index, `vector_top_k()` | FTS5 (thừa kế SQLite) | Điểm cộng lớn của quyết định chọn libSQL làm engine server. Giới hạn: index đòi ROWID table/PK đơn cột. |
...
```
   ...
   Kysely → better-sqlite3/libSQL/pg/mysql            Kysely → sqlite-wasm + OPFS
...
Server-side: `hooks` (trước/sau CRUD từng Entity, schedule, lifecycle) · `endpoints` (route API mới) · `jobs` (chạy trong Threads).
...
3. **CRDT log → SQL projection** — thứ tự replay, schema migration trên log cũ, compaction log. Cần một tài liệu thiết kế riêng trước khi code (docs/sync-design.md).
4. **Kysely vendor** — vendor thì phải theo được security fix upstream. Kế hoạch: script sync có kiểm tra diff, pin theo tag.
5. **Turso DB (Limbo)** đang beta — chỉ theo dõi, không đặt cược (§4.5).
...
- **Phase 1 — Kernel**: tách các module dùng chung từ akao thành kernel Nexus (UI, States/Context, Threads, Router, Events, FS/OPFS/IDB, SQL worker, Test) — akao sau này trở thành một *app chạy trên Nexus* (dogfood lớn nhất có thể).
...
- **Phase 4 — App system**: manifest + extension points + contract test + chính sách update §8.4 chạy thật.
