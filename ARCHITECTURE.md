...
- **Hợp đồng executor gồm cả transaction, không chỉ `run`/`all`.** `transaction(fn)` chạy toàn bộ callback trên **một** connection — commit khi return, rollback khi throw, và **không bao giờ** thay lỗi gốc bằng lỗi rollback. Ràng buộc "một connection" là lý do nó phải nằm trong hợp đồng chứ không để mỗi caller tự gõ `BEGIN`: driver có pool (`pg`, `mysql2`) phát một client bất kỳ cho **mỗi** query, nên `BEGIN` / thân / `COMMIT` rơi vào ba connection khác nhau và transaction thành vô nghĩa — im lặng, và chỉ trên engine dùng pool. sqlite/turso mở bằng `BEGIN IMMEDIATE` (transaction deferred chỉ lấy write lock ở lần ghi đầu, nên đọc-rồi-ghi vẫn hở). Không lồng nhau ở v1 (`E_NESTED_TX`): cú pháp savepoint khác nhau theo engine, và lồng-mà-làm-phẳng sẽ sinh ra transaction commit nửa chừng. Khả năng này được **khai báo** trong `CAPABILITIES.transactions`, tách bạch với `transactionalDDL` — MySQL có cái trước, không có cái sau. Điều khiển luồng transaction là logic thuần nên nằm ở tầng browser-safe (`Data/transaction.js`); chỉ connection thật mới ở sau ranh giới driver.
