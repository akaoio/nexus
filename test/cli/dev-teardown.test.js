...
Test.describe("dev teardown & reload hygiene (DEVFD/DEVDOWN)", () => {
    ...
    Test.it("DEVFD-03 the JOB RUNNER survives a hot reload — measured by running a job on the other side of one", async () => {
        // A regression this suite's own reordering introduced, and nothing
        // caught: `Threads` is a module-level registry keyed by name, and
        // register() returns the EXISTING thread for a name it already holds.
        // Building the new instance while the old runner was still registered
        // handed the new instance the OLD worker — old apps, old config — and
        // then took it away when the old instance was released. Every reload,
        // silently. A job enqueued afterwards sticks in `running` forever, and
        // nothing anywhere says the runner is gone.
        //
        // Driven end to end because that is the only way it shows: the runner
        // is not something a status endpoint reports on, so the observable is
        // a job that actually completes.
    ...
    endpoint("POST", "fire", async () => { await enqueue("probe.mark", { n: 1 }); return { queued: true } })`
        ...
        try {
            ...
            /** Enqueue one job and wait for the ledger to show it finished. */
            const runsAJob = async () => {
                const before = await (await fetch(base + "/api/v1/nexus_job")).json()
                const done = () => (before.ok ? before.data : []).filter((r) => r.status === "done").length
                const was = done()
                await fetch(base + "/api/v1/_/fire", { method: "POST", headers: { "content-type": "application/json" }, body: "{}" })
                for (let i = 0; i < 40; i++) {
                    const now = await (await fetch(base + "/api/v1/nexus_job")).json()
                    const rows = now.ok ? now.data : []
                    if (rows.filter((r) => r.status === "done").length > was) return true
