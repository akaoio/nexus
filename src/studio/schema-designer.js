...
import "./form-builder.js" // registers <nx-form-builder>
...
const STYLE = () => css`
    ...
    .panel { border-top: 2px solid #94a3b8; margin-top: 10px; padding-top: 8px }
    .change { display: flex; gap: 8px; align-items: center; margin: 3px 0 }
    .badge { font-size: 12px; padding: 1px 8px; border-radius: 999px; color: #fff }
    .badge.additive { background: #16a34a }
    .badge.structural { background: #dc2626 }
    .verdict { font-weight: 600; margin: 6px 0 }
    .verdict.hot { color: #16a34a }
    .verdict.migration { color: #dc2626 }
    .muted { color: #64748b }
    select, code { font: inherit }
    select { padding: 2px 6px; border: 1px solid #94a3b8; border-radius: 4px; background: transparent; color: inherit }
    code { background: #8882; padding: .1em .4em; border-radius: 4px; font-family: ui-monospace, monospace; font-size: 12px }
`
...
export class NxSchemaDesigner extends Component {
    ...
    mount() {
        render(html`
            ${STYLE()}
            <div ${({ element }) => (this.$editor = element)}></div>
            <div class="panel" ${({ element }) => (this.$panel = element)}></div>
        `, this.shadowRoot)
