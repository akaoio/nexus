...
import { defaultValue } from "./query-builder.js"
...
const STYLE = () => css`
    ...
    .row { border: 1px solid #94a3b833; border-radius: 6px; padding: 6px }
    input, select, button { font: inherit; padding: 2px 6px; border: 1px solid #94a3b8; border-radius: 4px; background: transparent; color: inherit }
    input.name { width: 9em } input.label { width: 9em } input.extra { width: 9em }
    button { cursor: pointer }
    button.remove { border-color: #dc2626; color: #dc2626 }
    .muted { color: #64748b }
    .invalid { outline: 2px solid #dc2626 }
    .required-mark { color: #dc2626 }
    form .field { display: flex; gap: 8px; align-items: center; margin: 6px 0 }
    form label { min-width: 10em }
    .preview { border-top: 1px dashed #94a3b8; margin-top: 10px; padding-top: 8px }
`
...
export class NxForm extends Component {
    ...
    mount() {
        render(html`${STYLE()}<form ${({ element }) => (this.$form = element)}></form>`, this.shadowRoot)
        ...
        render(html`
            ...
            <div class="head">
                ...
                <label>Label <input class="entity-label" ${({ element }) => {
                    ...
                    this.listen(element, "input", () => {
                        if (element.value === "") delete schema.label
                        else schema.label = { ...(schema.label ?? {}), en: element.value }
                        this.#emit()
                    })
                }}></label>
                <button class="add-field" ${({ element }) =>
            ...
            <div class="rows" ${({ element }) => (this.$rows = element)}></div>
            <div class="preview">
                <span class="muted">Live preview (nx-form rendering this schema):</span>
                <div ${({ element }) => (this.$previewSlot = element)}></div>
            </div>
        `, this.shadowRoot)
