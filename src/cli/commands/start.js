...
export async function start(args, flags, out) {
    ...
    let api, authState, challenges, engine, authMode
    try {
        ;({ api, authState, challenges, engine, authMode } = await buildInstanceApi({ root, config, schemas, apps, appPolicies, mode: "production" }))
    } catch (error) {
