...
**Green: 769/819 node clauses, 0 red** (`node test.js`)
...
| **The job runner survives a hot reload** | **a regression the reload reordering above introduced, and nothing caught: `Threads` is a module-level registry keyed by NAME, and `register()` returns the EXISTING thread for a name it already holds. Building the new instance while the old runner was still registered handed the new instance the OLD worker — old apps, old config — and then took it away entirely when the old instance was released. Every reload, silently. Measured end to end: a job enqueued before a hot reload completes; the next one after it sticks in `running` forever, and nothing anywhere says the runner is gone. The old effect runner is now stopped BEFORE the rebuild, alone; the API, the plane and the database handle still stay live until the new instance exists, so a rebuild that throws leaves the server answering. What a failed rebuild now costs is the job runner, which is the honest half of the trade and the reason DEVFD-02 claims a serving instance rather than a fully running one** | **DEVFD-03** |
