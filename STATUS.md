...
| **Realtime recovery** | **STATUS said a client that misses events "recovers by refetching" — and nothing made it refetch. The shared EventSource reconnects transparently, but subscribers heard nothing, so a list route subscribed across a network blip (a lid closing, a proxy idle timeout, a phone changing networks) showed stale data indefinitely, until some unrelated later event happened to arrive. Silent, because the page looks fine and is wrong. A link that dropped and came back now notifies every subscriber once, past the entity filter and past the dedupe set — whose key is entity/id/ts, which a resync does not have and which would collapse every one of them into a single swallowed event. The distinction is exact: a connection replaced DELIBERATELY, because a route mounting changed the entity union, covers no gap and stays quiet, or every navigation would reload every list — but a replacement DURING an outage does not erase the gap either, which is the case that is easy to get backwards. The decision is a three-input state machine, extracted so Node can drive it exhaustively where there is no EventSource at all** | **EVTSYNC-01..04** |
| **Sync transport: the peer list is the peer list** | **ZEN joins a LAN multicast group by DEFAULT, so a transport told to talk to `peers: ["http://relay/zen"]` also gossiped every event to anything on the local network that spoke the protocol and knew the channel name — nobody asked for that, and on a laptop in a café "sync with my relay" quietly meant "and broadcast to the room". It was breaking convergence too, which is how it was found: roughly half of two-peer runs on the development machine LOST an event permanently. Not delayed — a peer waited 150 seconds, seven times the harness's own window, with nothing in quarantine and no gate having rejected it. Never delivered. CI never saw it because a CI container has no LAN peers to discover, so only the relay path ever existed there, and a green CI is exactly why it went unexamined for so long: the suite looked flaky on one machine rather than wrong everywhere. Multicast is now opt-in, behind a pure options function so every case is a clause, and nothing short of an explicit `true` switches a network surface on** | **SYNCNET-01, ZSYNC-01..05** |
| **One instance, one thread namespace** | **a runner that died on every hot reload went unnoticed because a job runner is not something a status endpoint reports on — no clause could see it without enqueuing a job on the OTHER SIDE of a reload. `Threads` keys by NAME and `register()` is a get-or-create, so the constants `"job"`/`"plane"` made two live instances impossible, and dev builds the new instance before releasing the old one deliberately (a rebuild that throws must break nothing). The rebuilt instance was handed the OLD worker and lost it when the old one was released; a job enqueued afterwards sat in `running` forever. Names are minted INSIDE `startJobThread`/`bindPlaneRpc` — a caller asked to invent one is a caller that can forget, and forgetting reproduces the defect in silence — and "no plane" is `null` rather than a second default that would have to agree with the first. Two further defects came out of measuring rather than reasoning: saving a model reloads TWICE (the route, then the watcher noticing the file the server just wrote) and those raced, leaving one whole sqlite connection unreferenced per save — 40 descriptors over 24 reloads before chaining them, flat at 2 after; and `execute` charged the worker's cold start to the HANDLER's budget, so a recycle made the next innocent job report E_TIMEOUT and recycle again, walking healthy jobs into the DLQ** | **THRNS-01..03, DEVFD-01/03, JOB-TIMEOUT-04** |
...
- **Realtime event replay is WITHDRAWN, not deferred.** This used to read as a feature waiting
  its turn. The wire carries `{entity,event,id,ts}` and never row data, so an event is a
  notification to refetch — and a refetch already supersedes any replay of those, with the
  current truth rather than a history of intermediate states. Replay would also cost retention
  the hub deliberately does not have, plus a decision about whose visibility applies to
  historical events, which is the exact shape of the I11 `after:remove` id leak already closed
  here. What it stood in for now exists: a link that dropped and came back tells every
  subscriber to refetch (EVTSYNC-*). Reconnecting clients still refetch the full list rather
  than resuming from a checkpoint, and that is the design, not a gap.
