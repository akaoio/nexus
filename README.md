...
**All six core phases are complete**, built spec-first throughout — every contract was written as a red conformance clause *before* its implementation, and the whole tree is green: **430 conformance clauses, 0 red** — 385 executed in Node (`npm test`) and 45 in a real headless Chromium (`npm run test:browser`; those 45 report as browser-skipped in the Node run).
...
- **Engine choice** — `database.engine` in `nexus.config.json` selects sqlite (built-in, zero-install default), turso, postgres or mysql; a missing driver fails with the exact `npm install …` command.
- **Studio** — `<nx-query-builder>` (recursive, unlimited-depth, fuzzed in a real browser), `<nx-form-builder>`/`<nx-form>`, `<nx-schema-designer>` (live additive-vs-structural classification → a real migration document), `<nx-permission-manager>` (the first reuse of the query builder for row rules), `<nx-list-view>`, `<nx-search>`.
- **The app system** — versioned App Manifest (engines-gated), extension points (hooks/endpoints/commands into the Data Plane, HTTP and CLI), the full CLI (`create/dev/test/migrate/site/app/doctor`) with an additive `site restore` that never deletes destination data, and interim API-key auth with app-policy role assignment.
- **Sync** — the ZEN event log → SQL projection: content-addressed, secp256k1-signed events; HLC total order; row-level refold with **confluence proven** (any arrival order → byte-identical tables); four verification gates with quarantine-and-heal.
- **Semantic** — schema-declared serialization, a pluggable local-first embedding provider, text/vector/hybrid search that ranks **inside** permission, and RRF fusion in core.

**What is deliberately deferred** — and why — is listed in full at the end of [ARCHITECTURE.md §12](ARCHITECTURE.md): the live multi-engine CI matrix (needs real Postgres/MySQL/Turso services), the custom sqlite-wasm build with FTS5 + sqlite-vec and live ANN, the PEN graph gate and ZEN network transport, checkpoint pruning with super-peer roles, the full ZEN challenge-sign auth flow, and the NL→AST layer. None is a dead end: each has a seam already in the code, and every public format and API contract is frozen.
