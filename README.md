...
**Phase 1 complete — the kernel is extracted; 268 clauses, 0 red.** Every kernel module planned in ARCHITECTURE.md L1 now lives in `src/kernel/` (~60 files), each pinned by KRN-* clauses: the isomorphic core below, the UI engine, the full storage stack (FS with its pluggable format registry, OPFS, IDB, the SQL facade + WASM worker), and the HMR runtime (dev-browser-only by design, inert everywhere else). **Phase 2 — the Data Plane — is complete** (one honest caveat below). Kysely 0.29.3 is vendored (pinned, sha512-verified, resynced by a zero-dep script) behind `src/data/kysely.js` — a boundary enforced *statically* by the test suite. The **AST→Kysely compiler** is in and has earned the golden invariant: for any resolved AST document, rows selected by the compiled SQL on a **real SQLite engine** equal the rows kept by the reference JS predicate — proven over every operator on null-laden fixtures, the three-valued-logic NOT trap (each leaf compiles two-valued, so boolean algebra holds structurally), and 120 seeded random documents. Dialect care is pinned too: ILIKE on Postgres, explicit `ESCAPE` on SQLite, Turso riding the sqlite compiler.

The full runtime vertical is now alive: **Model→DDL** (entities become real tables — ULID identity, no DB-level FKs by CRDT design), the **Migration Engine** (hot DDL for what a dialect can truly do live; the universal rebuild with dry-run-by-default, data-loss reports, human-declared renames and an idempotent ledger for the rest), the **Data Plane CRUD API** (every access chains Model validation, permission resolution, AST injection and post-image checks — missing and forbidden are indistinguishable), and the **auto-generated HTTP API**: `nexus dev` now serves `GET/POST /api/v1/:entity`, `GET/PATCH/DELETE /api/v1/:entity/:id`, and `POST /api/v1/:entity/query` accepting a full Query AST document — straight from your schemas, permission-enforced at the Data Plane, pinned end-to-end over real HTTP.

**Engine adapters** close the phase: `createExecutor(engine)` puts real drivers behind the executor contract — sqlite via the built-in `node:sqlite` (the zero-install default, fully pinned including file persistence), and turso/postgres/mysql resolving their user-installed drivers dynamically from the instance (a missing driver fails with the exact `npm install …` command, never a cryptic module error). `nexus dev` reads `database.engine` from `nexus.config.json` — the Strapi-style engine choice is real. *The honest caveat:* live turso/postgres/mysql runs need real services — that multi-engine CI matrix is the one Phase 2 item deferred to CI infrastructure; their adapters follow the drivers' published APIs and their failure paths are pinned today. **Phase 3 has begun with the browser test runner**: `npm run test:browser` drives the browser-marked clauses in a real headless Chromium over raw CDP (Node's built-in WebSocket — still zero dependencies), serving the repo's ES modules over HTTP and reading the verdict the test page publishes. All 11 browser clauses are green — UI render/Component, OPFS, IndexedDB, and the HMR runtime, executed in a real browser. The run immediately paid for itself: it caught a latent rendering bug inherited from akao (a TreeWalker stops when its current node is detached, so templates with two or more sibling-position markers silently lost every marker after the first) — fixed in the kernel, pinned forever by KRN-UI21.

**`<nx-query-builder>` is alive** — the number-one differentiator, running in a real browser. A recursive Web Component (`nx-query-group` renders itself) that reads and writes real Query AST documents: unlimited nesting depth is structural, not a feature flag. Schema-aware condition rows (operators follow the field type, typed value editors — select options, boolean, numeric `in`-lists, `between` bounds), a NOT toggle per group (normalization pinned semantically identical via the reference predicate), pruning that keeps every reachable state valid, and byte-identical round-trips for documents without `not`. Its browser clauses include a seeded 60-action UI fuzz — **every reachable editor state must validate** — and ten nesting levels built with real clicks. The same component will drive list filters, the report builder, permission rules and validation rules (one UI, learned once). The isomorphic kernel core is in: environment detection, `Events` (one bus, both platforms), `States`/`Context` (proxy-based reactive state, decoupled from akao's app concerns), `Router` (pure pattern matching, no global reads), `Thread`/`Threads` (real Web Worker / worker_threads protocol with manager relay, pinned by tests that run actual Node workers), and the **UI engine** — `html()` (pure tagged-template → TemplateResult, fully pinned in Node), `render()`/`css()`/`Component` (browser-marked clauses await the browser runner). The kernel carries exactly **one reactivity system** (States + Events) by architectural rule.

The **`nexus` CLI skeleton** is alive — zero-dependency, bound by the §5.2 rules (thin shell over public APIs, `--json` everywhere as a versioned contract, plain output when piped, never overwrites existing data):

```bash
npx nexus create my-app --site "My App"   # scaffold an instance (validated via the public Model API)
cd my-app
nexus test                                # validate every schema — CI-ready exit codes
